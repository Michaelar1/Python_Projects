...
def center_window(self, w, h): #    Pass in the Tkinter frame (master) reference and the w and height
...
def onSelect(self, event):
    ...
    with conn:
        ...
        #   This returns a tuple and we can slice it into four parts using data[] during the iteration
        for data in varBody:
...
def addToList(self):
    ...
    var_fname = var_fname.title() # This will ensure that the first character in each entry is capitalized
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): #   Enforce the user to provide both names
        ...
        with conn:
            ...
            if chkName == 0: #  If there is a 0 then ther is no existance of the fullname
                ...
                self.lstList1.insert(END, var_fullname) #   Update the listbox with the name
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(self) #   call the function to clear all of the textboxes and the selected index of listboxes
...
def onUpdate(self):
    try:
        var_select = self.lstList1.curselection()[0] #  index sof the list seleciton
    ...
    #   For name changes, the suser will need to delete the entire record and start over.
    var_phone = self.txt_phone.get().strip() #  Normalize the data to maintain database integrity.
    var_email = self.txt_email.get().strip()
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present to update
        conn = sqlite.connect('db_phonebook.db')
        with conn:
            cur = conn.cursor()
            #   count records to see if the user's changes are already in
            #   the datavase...meaning there are no changes to update
            ...
            print(count2)
            if count == 0 or count2 == 0: # if proposed changes are already in the database, then proceed
                reponse = messagebox.askokcancel("Update Request", "The following changes ({}) and ({}) wil be implemented for ({}). \n\nProceed with the update request?".format(var_phone, var_email, var_value))
                print(response)
                if response:
                    #   conn = sqlite3.connect('db_phonebook.db')
                    with conn:
